        ...
        html += `<tr class="group-row" data-post-id="${postId}">
          <td colspan="7">
            ...
            <span style="float:right;display:flex;gap:6px;align-items:center">
              ...
              <button class="btn btn-danger btn-sm" onclick="removeGroupLinks('${postId}')" title="Remove all links in this post (keep anchor text)">Remove All</button>
    ...
    return `
      ...
      <td style="width:170px">
        <div style="display:flex;gap:4px;flex-wrap:wrap">
          ...
          <button class="btn btn-danger btn-sm" onclick="removeSingleLink(${link.post_id},'${escAttr(link.url)}',this)" title="Remove link (keep anchor text)">Remove</button>
        </div>
      </td>`;
}
...
/* ── Remove ONE link inline (unwrap anchor, keep text) ── */
async function removeSingleLink(postId, url, btn) {
    if (!confirm(`Remove this link?\n\n${url}\n\nThe anchor text stays — only the hyperlink is removed.`)) return;

    btn.disabled = true;
    btn.innerHTML = '<span class="spin spin-dark"></span>';

    try {
        const res = await fetch(`${API}?action=remove_link`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ post_id: postId, old_url: url }),
        });
        const data = await res.json();

        if (data.status === 'success') {
            removeLinkFromState(postId, url);
            showToast('Link removed!', 'success');
            buildGroups();
            updateStats();
            renderTable();
        } else if (data.status === 'no_change') {
            showToast('Link not found in post content', 'info');
            btn.disabled = false;
...
/* ── Bulk remove selected links ─────────────── */
async function bulkRemoveLinks() {
    if (!state.selected.size) { showToast('Select links first', 'error'); return; }
    if (!confirm(`Remove ${state.selected.size} selected link(s)?\n\nAnchor text stays — only the hyperlinks are removed.`)) return;

    const removals = [...state.selected].map(key => {
        const [postIdStr, oldUrl] = key.split('||');
        return { post_id: parseInt(postIdStr, 10), old_url: oldUrl };
    });

    const btn = document.getElementById('btn-bulk-remove');
    btn.disabled = true;
    btn.innerHTML = '<span class="spin"></span> Removing…';
    await runBulkRemove(removals);
    btn.disabled = false;
    btn.textContent = '✕ Remove Links';
}

/* ── Remove all links in one post group ─────── */
async function removeGroupLinks(postId) {
    const group = state.groups[postId];
    if (!group) return;
    if (!confirm(`Remove all ${group.links.length} link(s) in "${group.title}"?\n\nAnchor text stays — only the hyperlinks are removed.`)) return;

    const removals = group.links.map(l => ({ post_id: parseInt(postId, 10), old_url: l.url }));
    await runBulkRemove(removals);
}

/* ── Bulk remove helper ─────────────────────── */
async function runBulkRemove(removals) {
    try {
        ...
        // Drop removed links from in-memory state
        for (const r of data.results || []) {
            if (r.status === 'removed') removeLinkFromState(r.post_id, r.old_url);
        }

        showToast(`Removed ${data.removed} of ${data.total} link(s)`, 'success');
        state.selected.clear();
        buildGroups();
        updateStats();
        renderTable();
        ...
        showToast('Bulk remove failed: ' + e.message, 'error');
    }
}

/* ── Drop a link from state.links ───────────── */
function removeLinkFromState(postId, url) {
    state.links = state.links.filter(
        l => !(String(l.post_id) === String(postId) && l.url === url)
    );
